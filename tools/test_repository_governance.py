...
class RepositoryGovernanceTest(unittest.TestCase):
    def test_windsurf_skill_distribution_matches_canonical_files(self) -> None:
        canonical = ROOT / "skills"
        mirror = ROOT / ".windsurf" / "skills"
        files = sorted(
        ...
        for relative in files:
            with self.subTest(relative=relative):
                self.assertEqual((canonical / relative).read_bytes(), (mirror / relative).read_bytes())
    ...
    def test_changelog_declares_the_v81_twenty_one_skill_baseline(self) -> None:
        changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
        current_header = "\n".join(changelog.splitlines()[:12])

        self.assertIn("Third Brain V8.1 Skills", current_header)
        self.assertIn("Current V8.1 baseline: 21 Agent Skills.", current_header)
    ...
    def test_installers_are_v81_manifest_driven(self) -> None:
        for installer in (ROOT / "install.ps1", ROOT / "install.sh"):
            text = installer.read_text(encoding="utf-8")
            with self.subTest(installer=installer.name):
                self.assertIn("Third Brain V8.1 Skills Installer", text)
                self.assertIn("install_skills.py", text)
        self.assertTrue((ROOT / "tools" / "install_skills.py").is_file())

    def test_legacy_compatibility_scripts_have_no_personal_default_or_child_runner(self) -> None:
        scripts = (
            "worker_flow_engine.py",
            "run_30min_clippings_pipeline.py",
            "multi_agent_vault_team.py",
            "auto_heal_vault.py",
            "adapt_skills_to_vault.py",
            "upgrade_skills_v81.py",
        )
        for name in scripts:
            text = (ROOT / "tools" / name).read_text(encoding="utf-8")
            with self.subTest(script=name):
                self.assertNotIn("DEFAULT_VAULT", text)
                self.assertNotIn("OBSIDIAN_VAULT_PATH", text)
                self.assertNotIn("subprocess", text)
                self.assertNotIn("unittest", text)

    def test_current_guides_use_v81_main_and_the_hardened_installer(self) -> None:
        ...
        self.assertIn("PowerShell 5.1+", guide)
        self.assertTrue((ROOT / "install.ps1").is_file())
        self.assertIn("Third Brain V8.1", discovery)
        self.assertIn("21 profile-aware Agent Skills", discovery)

    def test_active_skill_standard_and_command_centre_use_v81_codex_baseline(self) -> None:
        active_surfaces = (
            ROOT / "README.md",
            ROOT / "docs" / "agent-skills-standard.md",
        ...
        for surface in active_surfaces:
            ...
            with self.subTest(surface=surface):
                self.assertIn("V8.1", text)
                self.assertNotIn('version: "7.1.0"', text)
                self.assertNotIn("V7 promotion gate", text)

        navigator = (ROOT / "tools" / "index.html").read_text(encoding="utf-8")
        self.assertIn("THIRD BRAIN V8.1 · CODEX HOST/KERNEL", navigator)
        self.assertIn("/api/snapshot", navigator)
        self.assertIn("repo.skill_count", navigator)
        self.assertNotIn("Skills (20)", navigator)
        self.assertEqual(len([path for path in (ROOT / "skills").iterdir() if path.is_dir()]), 21)
